...
import type { SystemInfo, FileNode, SearchResult, BacklinkInfo, GraphData, SemanticResult, AppSettings, OllamaHealth, GhostLink, GhostNotePreview, GhostSource } from './types';
...
export function vaultSetLastFile(systemPath: string, filePath: string): Promise<void> {
  return invoke('vault_set_last_file', { systemPath, filePath });
}

export function vaultGetConfig(systemPath: string): Promise<{ last_file?: string }> {
...
// Search
export function search(systemPath: string, query: string): Promise<SearchResult[]> {
...
export function getBacklinks(systemPath: string, noteName: string): Promise<BacklinkInfo[]> {
...
export function findNote(systemPath: string, name: string): Promise<string | null> {
  return invoke('find_note', { systemPath, name });
}

// Semantic Search
export function semanticSearch(systemPath: string, query: string): Promise<SemanticResult[]> {
...
export function previewGhostNote(systemPath: string, target: string, sources: GhostSource[]): Promise<GhostNotePreview> {
...
export function previewGhostNoteStream(
  ...
  return invoke('preview_ghost_note_stream_cmd', { systemPath, target, sources, onChunk });
}

export function createGhostNotes(systemPath: string, notes: GhostNotePreview[]): Promise<void> {
